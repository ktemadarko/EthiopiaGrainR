...
#' Download Costs form entries using EpiCollect5 API
#'
#'
...
#' \dontrun{
#' Costs_form <- costs_form_csv()
#' }

costs_form_csv<-function(){
  ...
  costs_form<-purrr::map_df(4,get_form) #fourth form from authorization details data frame
  #paste0("The Costs data frame has ", nrow(costs_form), " rows and ", ncol(costs_form)," columns")

  readr::write_csv(costs_form,"Costs_form.csv")
