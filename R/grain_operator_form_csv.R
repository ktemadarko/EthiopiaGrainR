...
#' Download Grain operator form entries using EpiCollect5 API
#'
#'
...
#' @rdname grain_operator_form_csv
#'
#' @return The dataset is written as a csv file in the IKEA_Ethopian_grain_mill_data folder on local storage
#' @export
#'
#' @examples
#' #Download and store the Grain operator form of the IKEA Ethiopia grain mill project in CSV
#' \dontrun{
#' Grain_operator_form<- grain_operator_form()
#' }

grain_operator_form_csv<-function(){
  ...
  grain_operator_form <-purrr::map_df(1,get_form)

  readr::write_csv(grain_operator_form,"Grain_operator_form.csv")

  #paste0("The grain operator data frame has ", nrow(grain_operator_form), " rows and ", ncol(grain_operator_form)," columns")
  return(grain_operator_form)
}
