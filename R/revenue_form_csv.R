...
#' Download Revenue form using EpiCollect5 API
#'
#'
...
#' @rdname revenue_form_csv
#'
#' @return The dataset is written as a csv file in the IKEA_Ethopian_grain_mill_data folder on local storage
#' @export
#'
#' @examples
#' #Download and store the Revenue form of the IKEA Ethiopia grain mill project in CSV
#' \dontrun{
#' Revenue_form <- revenue_form_csv()
#' }

revenue_form_csv<-function(){
  ...
  get_data <- function(pg_number) {

    api_url <-  paste0(form_api_links[2], "&format=json&page=", pg_number) #Revenue form is the 2nd form

    response <-httr::GET(api_url, httr::add_headers("Authorization" = paste("Bearer", token)))
  ...
  page_number_vec <- c(1:11) #pages 1-11
  revenue_form <- purrr::map_df(page_number_vec,get_data)

  return(revenue_form)

  readr::write_csv(revenue_form,"Revenue_form.csv")
